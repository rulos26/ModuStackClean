...
try {
    switch ($accion) {
        ...
        case 'organizacion_completa':
            echo json_encode(organizacion_completa());
            break;
...
function listar_archivos() {
    ...
    return array_slice($archivos, 0, 50); // Limitar a 50 archivos
}
...
function buscar_archivos($busqueda) {
    ...
    return array_slice($archivos, 0, 20); // Limitar a 20 resultados
}
...
function organizacion_completa() {
    $resultado = organizar_archivos();
    
    // También limpiar archivos antiguos
    $limpieza = limpiar_archivos(90); // 90 días
    
    return [
        'organizados' => $resultado['organizados'] ?? 0,
        'duplicados' => $resultado['duplicados'] ?? 0,
        'errores' => ($resultado['errores'] ?? 0) + ($limpieza['eliminados'] ?? 0),
        'archivos_eliminados' => $limpieza['eliminados'] ?? 0,
        'espacio_liberado' => $limpieza['espacio_liberado'] ?? '0 B'
    ];
}
...
function obtener_info_sistema() {
    ...
    $info['Servidor Web'] = $_SERVER['SERVER_SOFTWARE'] ?? 'Desconocido';
    ...
    // Tiempo de ejecución
    $info['Tiempo de Ejecución'] = round(microtime(true) - $_SERVER['REQUEST_TIME_FLOAT'], 3) . 's';
    
    // Información del servidor
    $info['IP del Servidor'] = $_SERVER['SERVER_ADDR'] ?? 'Desconocida';
    $info['Puerto'] = $_SERVER['SERVER_PORT'] ?? 'Desconocido';
